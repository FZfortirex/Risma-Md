let handler = async (m, { conn, args, usedPrefix }) => {
    if (args.length > 0) {
        ...
        if (warning < 19) {
            ...
            conn.sendButton(ban, '*Kamu di warn oleh moderator, dan sekarang kamu punya ' + (warning + 1) + '.Ingat Jika kamu mendapat warn 20 kali kamu akan otomatis ke banned*', wm, 'Owner', usedPrefix + 'owner', null)
        } else if (warning == 19) {
            global.db.data.users[ban].banned = true
            global.db.data.users[ban].warning = 0
            m.reply('*Dia sudah kebanned, karena mendapatkan 20 warn*')
            conn.sendButton(ban, '*Kamu ke banned karena telah mendapatkan 20 kali warn*', wm, 'Owner', usedPrefix + 'owner', null)
        }
